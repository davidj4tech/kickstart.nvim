...
do
  ...
  -- Smarter diffs: histogram matches moved/edited code better than the default
  -- myers, and linematch re-aligns lines within hunks.
  vim.opt.diffopt:append 'algorithm:histogram'
  vim.opt.diffopt:append 'linematch:60'

  -- Dedicated provider python (kept out of project venvs).
...
do
  ...
  -- [[ UI toggles ]] on <leader>u (see the which-key group in section 4)
...
do
  ...
  -- <leader>ss is owned by sllm (see custom/plugins/sllm.lua). Use :Telescope
  -- to reach the builtin picker.
...
-- ============================================================
-- Appearance & clipboard overrides (after colorscheme + plugins)
-- ============================================================

-- Transparent background: let the terminal's background through.
...
-- Clipboard over OSC52 so yanks reach the local clipboard through ssh/tmux.
vim.g.clipboard = {
  name = 'OSC52',
  copy = {
